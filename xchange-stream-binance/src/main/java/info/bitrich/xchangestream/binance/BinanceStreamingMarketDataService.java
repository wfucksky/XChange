...
public class BinanceStreamingMarketDataService implements StreamingMarketDataService {
    ...
    private final Map<CurrencyPair, Observable<BinanceKline>> klineSubscriptions;
    ...
    public Observable<BinanceKline> getRawKline(CurrencyPair currencyPair, Object... args) {
        if (!service.isLiveSubscriptionEnabled()
                && !service.getProductSubscription().getKline().contains(currencyPair)) {
            throw new UpFrontSubscriptionRequiredException();
        }
        return klineSubscriptions.computeIfAbsent(
                currencyPair, s -> triggerObservableBody(rawKlineStream(currencyPair, args)).share());
    }
    ...
    // @Override
    public Observable<BinanceKline> getKline(CurrencyPair currencyPair, Object... args) {
        return getRawKline(currencyPair, args);
    }
    ...
    private String channelFromCurrency(CurrencyPair currencyPair, String subscriptionType, Object... args) {
        ...
        // } else if (BinanceSubscriptionType.KLINE.getType().equals(subscriptionType)) {
        //     //todo 调整 kline 监听
        //     if (args != null && args.length > 0) {
        //         KlineInterval interval = (KlineInterval) args[0];
        //         return currencyChannel + "_" + interval.code();
        //     } else {
        //         return currencyChannel + "_" + KlineInterval.m1.code();
        //     }
        } else {
    ...
    public void openSubscriptions(ProductSubscription productSubscription) {
        ...
        productSubscription.getKline().forEach(this::initKlineSubscription);
    ...
    private void initKlineSubscription(CurrencyPair currencyPair, Object... args) {
        klineSubscriptions.put(
                currencyPair, triggerObservableBody(rawKlineStream(currencyPair, args)).share());
    }
    ...
    private Observable<BinanceKline> rawKlineStream(CurrencyPair currencyPair, Object... args) {
        return service
                .subscribeChannel(
                        channelFromCurrency(currencyPair, BinanceSubscriptionType.KLINE.getType(), args))
                .map(
                        it ->
